...
int eeprom_read_byte(struct eeprom* e, __u16 mem_addr)
{
	...
	if(e->type == EEPROM_TYPE_8BIT_ADDR)
	{
		...
		r = i2c_write_1b(e, buf);	/* ��д1�ֽڵ�ַ��Ϣ */
	} else if(e->type == EEPROM_TYPE_16BIT_ADDR) {
		__u8 buf[2] = { (mem_addr >> 8) & 0x0ff, mem_addr & 0x0ff };
		r = i2c_write_2b(e, buf);	/* ��д2�ֽڵ�ַ��Ϣ */
	} else {
	...
	if (r < 0)
		return r;
	r = i2c_smbus_read_byte(e->fd);	/* �ٶ�1�ֽ�ʵ������ */
	return r;
}

int vr_read_byte(int file,__u16 mem_addr)
{
	int r;
	ioctl(file, BLKFLSBUF); // clear kernel read buffer
	__u8 buf =  mem_addr & 0x0ff;
	/*
	r = i2c_vr_write_1b(file, buf);	//锟斤拷写1锟街节碉拷址锟斤拷息 

	if(r < 0)
		return r;

	r = i2c_smbus_read_byte(file);	// 閿熷姭璁规嫹1閿熻鏂ゆ嫹瀹為敓鏂ゆ嫹閿熸枻鎷烽敓鏂ゆ嫹 
	*/
	
	r = i2c_smbus_read_byte_data(file,buf);
...
int eeprom_write_byte(struct eeprom *e, __u16 mem_addr, __u8 data)
{
	if(e->type == EEPROM_TYPE_8BIT_ADDR) {
		...
		return i2c_write_2b(e, buf);	/* i2c_write_2bд2�ֽڵ�ַ��Ϣ,1�ֽ�������Ϣ */
	} else if(e->type == EEPROM_TYPE_16BIT_ADDR) {
		__u8 buf[3] =
			{ (mem_addr >> 8) & 0x00ff, mem_addr & 0x00ff, data };
		return i2c_write_3b(e, buf);	/* i2c_write_3bд2�ֽڵ�ַ��Ϣ,1�ֽ�������Ϣ */
	} else {
		fprintf(stderr, "ERR: unknown eeprom type\n");
		return -1;
	}
